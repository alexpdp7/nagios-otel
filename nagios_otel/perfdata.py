...
def parse_host_perfdata(s):
    ...
    >>> parse_host_perfdata("[HOSTPERFDATA]\t1699561380\texample.com\t30.005\t(Host check timed out after 30.00 seconds)\t")
    HostPerfdata(timestamp=datetime.datetime(2023, 11, 9, 21, 23), host='example.com', host_execution_time=30.005, status='(Host check timed out after 30.00 seconds)', perfdata=[])
    """
...
def parse_metric(metric_str):
    ...
    return Metric(label, value, float(min_str) if min_str else None, float(max_str) if max_str else None, warn, crit, uom)
